# My function Continuous ---------------- 31012025


# # generate the 10000 datasets a
# load("C:/Users/AjsiKanapari/Unit of Biostatistics Epidemiology and Public Health/LACED - Randomizzation by PS - 1 - Randomizzation by PS - 1/script5_sct_paper/output/data_continuous.rda")
# # data_continuous
#
# load(file = "list_mod_continuous_20250505.rda")
# names(list_mod_continuous_outcome) <- c("glm_model", "rf_model", "nb_model", "xgboost_model", "sl_model")

# caliper_assigned <- 0.1
...
# sim=6
# delta_effect=0.5; N=20; iter=1; n=50; method="glm_model"; caliper_assigned = 0.05


# load(file = "data/deciles_list.Rda")
# names(quantiles_list) <- c("glm_model", "xgboost_model")
...
my_simulation_continuous_function <- function(delta_effect, N, iter, n, method, caliper_assigned = caliper_assigned) {
  ...
  for (sim in seq_along(1:iter)) {
    print(sim)
    # data from out
    data_RCT1 <- data_continuous[[sim]]  |> dplyr::group_by(T_equal) |>  dplyr::sample_n(size=n, replace = FALSE)
    ...
    for (method in methods) {
      ...
      if (method == "sl_model") {
        ...
        data_RCT1$subclass <- cut(data_RCT1$ps, quantiles_list[[method]])
      ...
      for (caliper_value in caliper_assigned) {
        ...
        subclasses <- unique(data_RCT1$subclass) # vettore di classi
        db_subclasses <- purrr::map( # ragruppati e filtrati per strato
          .x = subclasses, ~ data_RCT1 |>
        ...
        for (j in 1:length(subclasses)) {
          if (nrow(db_subclasses[[j]]) > 0) {  #
            #print(j)
            db_subclasses[[j]]$T_ml <- randomizr::complete_ra(N = nrow(db_subclasses[[j]])) # creato random 0.5 per strato
            db_merged <- rbind(db_merged, db_subclasses[[j]])
          }
        }

        m_dataRCT <- db_merged
        m_dataRCT$T_matched <- m_dataRCT$T_ml
        db_merged <- dplyr::right_join(data_RCT1, m_dataRCT[, c("id", "subclass", "T_matched")], by = c("id","subclass"))
      table(db_merged$T_equal, db_merged$T_matched)
        ...
        for (delta in (c(0, delta_effect))) {
          ...
          # unpaired glm
          test <- glm(response_T_ml ~ T_matched, data = db_merged)
          coef_ml <- coef(test)[2]
          sd_ml <- summary(test)$coefficients["T_matched", "Std. Error"]
          p_ml_sign <- summary(test)$coefficients["T_matched", 4] < 0.05
          ...
          # glmer
          if (length(unique(db_merged$subclass)) > 1) {
          mod_ml_lmer <- lmer(response_T_ml ~ T_matched + (1 | subclass), data = db_merged)
          } else {
          mod_ml_lmer <- glm(response_T_ml ~ T_matched, data = db_merged)
          }
          ...
          out <- data.frame(
            ...
            caliper_distance = NA_character_,
            smd_matched_ps = NA_character_,
            smd_matched_cov = NA_character_,
            method = method,
            initial_size = length(unique(db_merged$subclass)),
            reduced_size = size,
        ...
        db_merged$T_equal_random <- db_merged$T_equal
