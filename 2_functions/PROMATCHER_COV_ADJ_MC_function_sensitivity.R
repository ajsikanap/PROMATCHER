    ...
    for (method in methods) {
      ...
      for (caliper_value in caliper_assigned) {
        ...
        for (delta in (c(0, delta_effect))) {
          ...
          # glmer  ' non eà pi lmer ma glm adj per covariates

          mod_ml_lmer <- glm(response_T_ml ~ T_matched+V1+V2+V3+V4+V5+V6+V7+V8+V9+V10+V16+V17 , data = db_merged)
          coef_T_ml_lmer <-  coef(mod_ml_lmer)[2]
          sd_T_ml_lmer <- summary(mod_ml_lmer)$coefficients["T_matched", "Std. Error"]
          p_ml_sign_lmer <- summary(mod_ml_lmer)$coefficients["T_matched", 4] < 0.05
        ...
        for (delta in c(0, delta_effect)) {
          ...
          out_equal <- data.frame(
            ...
            # lmer subclass
            p_sign_lmer = p_equal_sign_lmer, # glmer
  ...
  for (i in seq_along(sample_size_to_try)) {
    power_levels_temp[[i]] <- power_levels[[i]]$final_output |>
      ...
      summarize(
        ...
        bias_ttest  = mean(coef_ttest - unique(delta), na.rm=TRUE),
        ...
        bias_glm_adj  = mean(coef_glm_adj - unique(delta), na.rm=TRUE),
        ...
        bias_lmer  = mean(coef_lmer - unique(delta),na.rm=TRUE),
