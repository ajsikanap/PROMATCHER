...
for (sim in seq_along(1:iter)) {
  ...
  er<-rnorm(N, mean = 0, sd = 8)
  
  y_pre_T_c <- score+er
  data <- cbind(y_pre_T_c, z,score,er) |>
    #  mutate_at(vars(1:11), ~scale(.)) |>
    as.data.frame()
  data$id <- seq(1, nrow(data))
  ...
  data_continuous2[[sim]]<-data_RCT1
  
}

save(data_continuous2, file="data/data_continuous_toy_example_N_10_score_known.rda")
#save(data_continuous2, file="data/data_continuous_10000_rcts_score_known.rda")
