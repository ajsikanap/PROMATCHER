...
# carica results good
load("output/tbl_results_good.rda")
continuous_table<-tbl_results_good

names(tbl_results_good)
names(continuous_table)<-c("Caliper", "n/2", "Model", "Analysis",
                           "equal_smd","paired_smd","Power_eq",
...
analysis_colors <- c(
  ...
  "Linear mixed model (pair RE)"   = "#CC79A7"  # pink
)
...
ggsave(
  filename = "figures/figure1_good.eps",   # nome file
  plot = figure1_good,        # oggetto del plot
  device = cairo_ps,          # usa Cairo per testo vettoriale pulito
...
ggsave(
  filename = "figures/figure1_good.jpeg",
  plot = figure1_good,
  device = "jpeg",
  width = 10,
  height = 8,
...
load("output/tbl_results_bad.rda")
...
ggsave(
  filename = "figures/figure1_bad.eps",   # nome file
...
ggsave(
  filename = "figures/figure1_bad.jpeg",
  plot = figure2_bad,
  device = "jpeg",
  width = 10,
  height = 8,
...
#
# ggsave(
#   filename = "figures/figure1_bad.png",
#   plot = figure2_bad,
#   device = "png",
#   width = 10,
#   height = 8,
...
# Figura 2 -----------
# con aggiustamento con in trial covariates
# Adjusted
load("output/tbl_caliper005_49_20_adjusted.rda")

# 1. Riformatta il dataset per plot publication-ready
plot_df <- tbl3_final %>%
  select(initial_size_mean, model, analysis,
         equal_power_alt, paired_power_alt) %>%
  pivot_longer(
    cols = c(equal_power_alt, paired_power_alt),
    names_to = "method",
    values_to = "power"
  ) %>%
  mutate(
    method = recode(method,
                    equal_power_alt      = "Equal randomization",
                    paired_power_alt = "PROMATCHER"),
    analysis = recode(analysis,
                      ttest   = "t-test",
                      glm_adj = "GLM adjustment",
                      trial_cov_adj     = "GLM trial covariate adj"),
    model = recode(model,
                   glm_model   = "GLM",
                   rf      = "Random Forest",
                   xgboost_model = "XGBoost",
  ...
  mutate(
    model = factor(model, levels = c("GLM", "Random Forest", "XGBoost", "Super Learner")),
    analysis = factor(analysis, levels = c("t-test", "GLM adjustment", "GLM trial covariate adj")),
    method = factor(method, levels = c("Equal randomization", "PROMATCHER"))
  )
...
analysis_colors <- c(
  "t-test" = "#0072B2",           # blu
  "GLM adjustment" = "#E69F00",    # arancione
  "GLM trial covariate adj" = "#CC79A7"  # verde
)
...
plot_df2 <- tbl3_final %>%   # sostituisci con il nome del tuo oggetto
  select(initial_size_mean, model, analysis,
         equal_power_null, paired_power_null) %>%
  pivot_longer(
    cols = c(equal_power_null, paired_power_null),
    names_to = "group",
    values_to = "typeIerror"
  ) |>
  mutate(
    group = ifelse(group == "equal_power_null", "equal", "paired"),
    method = recode(group,
...
ggsave(
  filename = "figures/figure2_cov_adj.eps",   # nome file
  plot = figure2,        # oggetto del plot
  device = cairo_ps,
  width = 10,                  # larghezza in pollici (adatta a colonna singola/doppia)
  height = 8,                 # altezza in pollici
  dpi = 600
...
ggsave(
  filename = "figures/figure2_cov_adj.jpeg",
  plot = figure2,
  device = "jpeg",
  width = 10,
  height = 8,
...
# ggsave(
#   filename = "figures/figure2_cov_adj.png",
#   plot = figure2,
#   device = "png",
#   width = 10,
#   height = 8,
...
# 3. FIGURA 3-------
# this output comes from ... table for paper, and it is for statified randomization
load("output/tbl_caliper005_49_20_strata.rda")
...
# 1. Riformatta il dataset per plot publication-ready
plot_df <- tbl4_final %>%
  select(initial_size_mean, sample_size, model, analysis,
  ...
  mutate(
    method = recode(method,
                    ...
                    paired_power_1 = "Stratified Randomization"),
    analysis = recode(analysis,
                      ttest   = "t-test",
                      glmadj = "GLM adjustment",
                      lmer    = "Linear mixed model (stratum RE)"),
    model = recode(model,
                   glm_model   = "GLM",
                   rf      = "Random Forest",
                   xgboost_model = "XGBoost",
  ...
  distinct(initial_size_mean, sample_size, model, analysis, method, .keep_all = TRUE) %>%
  mutate(
    model = factor(model, levels = c("GLM", "Random Forest", "XGBoost", "Super Learner")),
    analysis = factor(analysis, levels = c("t-test", "GLM adjustment", "Linear mixed model (stratum RE)")),
    method = factor(method, levels = c("Equal randomization", "Stratified Randomization"))
  )
...
analysis_colors <- c(
  "t-test" = "#0072B2",           # blu
  "GLM adjustment" = "#E69F00",    # arancione
  "Linear mixed model (stratum RE)" = "#CC79A7"  # verde
)
...
pw_small <- ggplot(plot_df, aes(
  ...
  scale_shape_manual(values = c(
    "Equal randomization" = 16,   # cerchio pieno
    "Stratified Randomization" = 22   # triangolo pieno
  )) +
  labs(
    title = "Power vs Sample Size by Model",
    x = "Sample size",
...
plot_df2 <- tbl4_final %>%   # sostituisci con il nome del tuo oggetto
  select(initial_size_mean, sample_size, model, analysis,
  ...
  mutate(
    ...
    method = recode(group,
                    equal  = "Equal randomization",
                    paired = "Stratified Randomization"),
    analysis = recode(analysis,
                      ttest   = "t-test",
                      glmadj = "GLM adjustment",
                      lmer    = "Linear mixed model (stratum RE)"),
...
ggsave(
  filename = "figures/figure3_stata.eps",   # nome file
...
ggsave(
  filename = "figures/figure3_stata.jpeg",
  plot = figure3,
  device = "jpeg",
  width = 10,
  height = 8,
...
# ggsave(
#   filename = "figures/figure3_strata.png",
#   plot = figure3,
#   device = "png",
#   width = 10,
#   height = 8,
...
p_se_good<-ggplot(df_plot_good, aes(x = sd, y = model, shape=method)) +
  ...
  xlim(0.061,0.085) +
  scale_shape_manual(values = c("Equal randomization" = 16, "PROMATCHER" = 2)) +
